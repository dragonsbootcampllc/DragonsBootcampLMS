  ...
  if (useRefs !== false) {
    for (const [, assoc] of Object.entries(model.associations)) {
      ...
      switch (associationType) {
        case 'HasOne':
        case 'BelongsTo':
          assSchema = options.ref != "" ? {$ref:`#/${options.ref}/${target.name}`} : {$ref: `#/definitions/${target.name}`};
          break;
        case 'HasMany':
        case 'BelongsToMany':
          assSchema = {
            type: 'array',
            items: options.ref != "" ? {$ref:`#/${options.ref}/${target.name}`} : {$ref: `#/definitions/${target.name}`}
          };
          break;
        default:
...
function getSequelizeSchema(seq, options = {}) {
  ...
  for (const [name, model] of Object.entries(seq.models)) {
    ...
    if(options.ref) mopts.ref = options.ref;
